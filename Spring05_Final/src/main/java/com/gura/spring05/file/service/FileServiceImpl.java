...
public class FileServiceImpl implements FileService{
	...
	public void saveFile(FileDto dto, ModelAndView mView,HttpServletRequest request) {
				//업로드된 파일의 정보를 가지고 있는 객체의 참조값 얻어오기
				...
				//webapp/upload 폴더 까지의 실제 경로 얻어내기
				...
				try {
					dto.getMyFile().transferTo(new File(filePath+saveFileName));			
				}catch(Exception e){
					e.printStackTrace();
				}
				// 업로드된 파일 정보를 fileDto에 담는다
				String id=(String)request.getSession().getAttribute("id");
				dto.setWriter(id);
	...
	public void deleteFile(HttpServletRequest request, int num, ModelAndView mView) {
		...
		String path=request.getServletContext().getRealPath("/upload")+File.pathSeparatorChar+saveFileName;
		new File(path).delete();
		boolean isdelete=filedao.delete(num);
